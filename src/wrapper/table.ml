...
let col (type a) (data : a array) (type_ : a col_type) ~name =
  ...
  | Span_ns -> span_ns data ~name
  | Ofday_ns -> ofday_ns data ~name
  | Bool ->
...
let col_opt (type a) (data : a option array) (type_ : a col_type) ~name =
  ...
  | Span_ns -> span_ns_opt data ~name
  | Ofday_ns -> ofday_ns_opt data ~name
  | Bool ->
    let bs = Valid.create_all_valid (Array.length data) in
...
let read (type a) t ~column (col_type : a col_type) : a array =
  ...
  | Time_ns ->
    ...
    | _ ->
      Wrapper.Column.read_utf8 t ~column |> Array.map ~f:Core.Time_ns.of_string_with_utc_offset)
  | Span_ns ->
